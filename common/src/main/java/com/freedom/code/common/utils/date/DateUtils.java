...
public class DateUtils {
	...
	public static final long TIME_OF_A_WEEK_DAY = 7 * 24 * 60 * 60 * 1000;
	...
	public static String addMonthsToDateTime(String date, int addMonths) throws ParseException {
		...
		c.add(Calendar.MONTH, addMonths);
		return fullDateTime(
				DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.MEDIUM, Locale.CHINA).format(c.getTime()));
	}
	...
	/**
	 * 
	 * Created on 2015年1月29日 Description:获取年月号(上午/下午)时分
	 * 
	 * @author:崔海英
	 * @param date
	 * @return
	 ...
	 * 
	 * 功能描述: <br>
	 * 获取传入的日期是哪一年的第几周，从周一算开始 2015-14
	 *
	 * @param date
	 * @return
	 * @see [相关类/方法](可选)
	 * @since [产品/模块版本](可选)
	 */
	public static String getYearAndWeekNum(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setFirstDayOfWeek(Calendar.MONDAY);
		calendar.setTime(date);
		return (1900 + date.getYear()) + "-" + calendar.get(Calendar.WEEK_OF_YEAR);
	}
	 ...
	 * @return
	 */
	public static Date getFirstMillsOfMonth(Date date, int nMonth) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
	...
	public static void main(String[] args) {
		/*
		 * SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_PATTERN); try
		 * { String dfString=dateFormat.format(new Date()); Date
		 * date=dateFormat.parse(dfString); System.out.println(dfString);
		 * System.out.println(date); //
		 * System.out.println(dateFormat.parse("2014-01")); //
		 * System.out.println(afterAll("2014-10-23 21:10:01",
		 * "2014-10-22 21:10:01")); } catch (ParseException e) { // TODO
