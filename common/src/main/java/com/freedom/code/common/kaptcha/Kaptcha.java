package com.freedom.code.common.kaptcha;

import java.awt.Color;
import java.awt.Font;
import java.io.OutputStream;

import com.freedom.code.common.ValidateUtils;
import static com.freedom.code.common.kaptcha.Randoms.alpha;
import static com.freedom.code.common.kaptcha.Randoms.num;

/**
 * 自定义验证码类
 * @author luo
 *
 */
public abstract class Kaptcha {
	
	protected Font font = new Font("Verdana", Font.ITALIC|Font.BOLD, 20);   // 字体
    protected int len = 5;  // 验证码随机字符长度
    protected int width = 150;  // 验证码显示跨度
    protected int height = 40;  // 验证码显示高度
    private String chars = null;  // 随机字符串
    
    /**
     * 生成随机字符数组
     * @return 字符数组
     */
    protected char[] alphas()
    {
    	if(!ValidateUtils.isNull(chars)){
    		return chars.toCharArray();
    	}else{
    		char[] cs = new char[len];
            for(int i = 0;i<len;i++)
            {
                cs[i] = alpha();
            }
            chars = new String(cs);
            return cs;
    	}
    }
    
    public String getChars(){
		return chars;
	}

	public void setChars(String chars){
		this.chars = chars;
	}

	public Font getFont(){
        return font;
    }
 
    public void setFont(Font font){
        this.font = font;
    }
 
    public int getLen(){
        return len;
    }
 
    public void setLen(int len){
        this.len = len;
    }
 
    public int getWidth(){
        return width;
    }
 
    public void setWidth(int width){
        this.width = width;
    }
 
    public int getHeight(){
        return height;
    }
 
    public void setHeight(int height){
        this.height = height;
    }
 
    /**
     * 给定范围获得随机颜色
     * @return Color 随机颜色
     */
    protected Color color(int fc, int bc){
        if (fc > 255)
            fc = 255;
        if (bc > 255)
            bc = 255;
        int r = fc + num(bc - fc);
        int g = fc + num(bc - fc);
        int b = fc + num(bc - fc);
        return new Color(r, g, b);
    }
 
    /**
     * 验证码输出,抽象方法，由子类实现
     * @param os 输出流
     */
    public abstract void out(OutputStream os);
 
    /**
     * 获取随机字符串
     * @return string
     */
    public String text(){
        return chars;
    }
}
